<?php
function wpnj_mediatags_wp_export_metadata()
{
	...
	if ($post_attachments)
	{
		...
		{
			$mediatag_post_terms = get_the_terms( $attachment->ID, WPNJ_MEDIA_TAGS_TAXONOMY );
...
function wpnj_mediatags_wp_import_metadata($post_id='', $key='', $value='')
{
	if (!$post_id) return;
	if (!$key) return;
	if (!$value) return;
	...
	if ($key == 'post_media_tags_export')
	{
		...
		if ($mediatag_meta_items)
			wp_set_object_terms($post_id, $mediatag_meta_items, WPNJ_MEDIA_TAGS_TAXONOMY);
	}
}
?>