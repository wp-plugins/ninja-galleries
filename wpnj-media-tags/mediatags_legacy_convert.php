...
function wpnj_mediatags_plugin_version_check()
{
	// We get the version from the main WP wp_options table. 
	$media_tags_version = get_option('wpnj-media-tags-version');
	...
	if (!$media_tags_version)
	{
		...
		$legacy_master_media_tags = wpnj_legacy_load_master_media_tags();
		if ($legacy_master_media_tags)
		{
			foreach($legacy_master_media_tags as $legacy_slug => $legacy_name)
			{
				if ( ! ($id = term_exists( $legacy_slug, WPNJ_MEDIA_TAGS_TAXONOMY ) ) )
					wp_insert_term($legacy_name, WPNJ_MEDIA_TAGS_TAXONOMY, array('slug' => $legacy_slug));
			}
		...
		if ($post_attachments)
		{
			foreach($post_attachments as $attachment)
			{
				...
				if ($legacy_media_meta)
				{
					...
					$legacy_post_media_tags_array = wpnj_legacy_get_post_media_tags($attachment->ID, $legacy_post_media_tags_str);
					if ($legacy_post_media_tags_array)
					{
						wp_set_object_terms($attachment->ID, $legacy_post_media_tags_array, WPNJ_MEDIA_TAGS_TAXONOMY);
					}
				}
			}				
			...
			foreach($post_attachments as $attachment)
			{
				$media_tags_tmp 	= (array)wp_get_object_terms($attachment->ID, WPNJ_MEDIA_TAGS_TAXONOMY);
			}
		}
		
		// Then insert/update the options table with the current plugin version so we don't have to check each time. 
		update_option('wpnj-media-tags-version', WPNJ_MEDIA_TAGS_VERSION);
	}
	else if ($media_tags_version < WPNJ_MEDIA_TAGS_VERSION)
	{
		// Here we might need to do something for other variations. 
	}
}

function wpnj_legacy_load_master_media_tags()
{
	//$master_images_tags_list = "One 1, Two 2, Three 3";
	$master_media_tags_list = get_option('wpnj-media-tags');
...
function wpnj_legacy_get_post_media_tags($post_id, $post_media_tags_list)
{
