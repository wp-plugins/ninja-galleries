<?php
function wpnj_mediatags_admin_panels()
{
	add_media_page( "Wpnj Media Tags", "Wpnj Media Tags", 'manage_options', WPNJ_ADMIN_MENU_KEY, 'wpnj_mediatags_mgmt_panel');
	add_options_page('Wpnj Media Tags', 'Wpnj Media Tags', 'manage_options', WPNJ_ADMIN_MENU_KEY, 'wpnj_mediatags_settings_panel');
}

function wpnj_mediatags_show_fields_to_edit($form_fields, $post) 
{	
	$post_media_tags_fields = wpnj_mediatags_get_fields($post->ID);
...
function wpnj_mediatags_get_fields($post_id)
{
	$master_media_tag_fields = "";
	
	$media_tags_tmp 	= (array)wp_get_object_terms($post_id, WPNJ_MEDIA_TAGS_TAXONOMY);
	...
	$master_media_tags_array = wpnj_mediatags_load_master();	
	if ($master_media_tags_array)
	{
		...
		if (strlen($master_media_tag_fields_tmp['used_item']))
			$master_media_tag_fields .= '<a id="wpnj-media-tags-show-hide-used" href="#">Media Tags for this attachment</a>
				<div id="wpnj-media-tags-list-used"><ul class="wpnj-media-tags-list">'. 
					$master_media_tag_fields_tmp['used_item'] . '</ul></div>';
		if (strlen($master_media_tag_fields_tmp['used_all']))
			$master_media_tag_fields .= '<a id="wpnj-media-tags-show-hide-common" href="#">Show Common Media Tags</a>
				<div id="wpnj-media-tags-list-common"><ul class="wpnj-media-tags-list">'. $master_media_tag_fields_tmp['used_all'] . '</ul></div>';
		if (strlen($master_media_tag_fields_tmp['unused_all']))
			$master_media_tag_fields .= '<a id="wpnj-media-tags-show-hide-uncommon" href="#">Show Uncommon Media Tags</a>
				<div id="wpnj-media-tags-list-uncommon"><ul class="wpnj-media-tags-list">'.$master_media_tag_fields_tmp['unused_all'] . '</ul></div>';


//		if (strlen($master_media_tag_fields))
//			$master_media_tag_fields = '<ul id="wpnj-media-tags-list">'. $master_media_tag_fields . '</ul>';
	}
	return $master_media_tag_fields;
}

function wpnj_meditags_process_attachment_fields_to_save($post, $attachment) 
{	
	...
	if (strlen($attachment['media_tags_input']))
	{
		...
		if ($tags_tmp_array)
		{
			foreach($tags_tmp_array as $idx => $tag_val)
			{
				...
				if ( ! ($id = term_exists( $tag_slug, WPNJ_MEDIA_TAGS_TAXONOMY ) ) )
					wp_insert_term($tag_val, WPNJ_MEDIA_TAGS_TAXONOMY, array('slug' => $tag_slug));
	...
	if ($media_tags_array)
	{
		wp_set_object_terms($post['ID'], $media_tags_array, WPNJ_MEDIA_TAGS_TAXONOMY);			
	}
	else
	{
		wp_set_object_terms($post['ID'], "", WPNJ_MEDIA_TAGS_TAXONOMY);				
	}
    return $post;
}

function wpnj_mediatags_load_master()
{
	$media_tags_tmp = (array) get_terms(WPNJ_MEDIA_TAGS_TAXONOMY, 'hide_empty=0');
	if ($media_tags_tmp)
	{
...
function wpnj_mediatags_delete_attachment_proc($postid = '')
{
	if (!$postid) return;
	
//	$tt_ids = wp_get_object_terms($postid, WPNJ_MEDIA_TAGS_TAXONOMY, 'fields=tt_ids');
//	echo "tt_ids<pre>"; print_r($tt_ids); echo "</pre>";
//	exit;
	wp_delete_object_term_relationships($postid, array(WPNJ_MEDIA_TAGS_TAXONOMY));	
	//wp_update_term_count( $postid, WPNJ_MEDIA_TAGS_TAXONOMY);
}

function wpnj_mediatags_mgmt_panel() {
	require_once(ABSPATH . 'wp-includes/pluggable.php');

	wpnj_media_tags_register_columns();
	...
	<div class="wrap nosubsub">
		...
		<?php
			if ((isset($_GET['action'])) && ($_GET['action'] == 'editmediatag'))
			{
				...
				wpnj_mediatags_process_edit($mediatag_ID);
			}
			else
			{
				?>
				...
				<p class="search-box">
					<input type="hidden" name="page" value="<?php echo WPNJ_ADMIN_MENU_KEY; ?>" />
					<input type="hidden" name="action" value="searchmediatag" />
					<label class="hidden" for="wpnj-media-tags-search-input"><?php _e( 'Search Media Tags' ); ?>:</label>
					<input type="text" class="search-input" id="wpnj-media-tags-search-input" name="s" value="<?php _admin_search_query(); ?>" />
				...
				<div id="col-container">
					<div id="col-right">
						<div class="col-wrap">
							<form id="posts-filter" method="get"
								...
								<div class="tablenav">
									<?php
										...
										$page_links = paginate_links( array(
											...
											'format' => '',
												'prev_text' => __('&laquo;'),
												'next_text' => __('&raquo;'),
												'total' => ceil(wp_count_terms(WPNJ_MEDIA_TAGS_TAXONOMY) / $tagsperpage),
												'current' => $pagenum
										));
									...
									<div class="alignleft actions">
										...
										<?php wp_nonce_field('wpnj-media-tags-bulk'); ?>
									</div>
							...
							<thead>
								<tr><?php print_column_headers('edit-wpnj-media-tags'); ?></tr>
							</thead>

							<tfoot>
							<tr><?php print_column_headers('edit-wpnj-media-tags', false); ?></tr>
							</tfoot>

							<tbody id="the-list" class="list:tag">
							<?php
								$searchterms = isset( $_GET['s'] ) ? trim( $_GET['s'] ) : '';
								$count = wpnj_media_tags_display_rows( WPNJ_MEDIA_TAGS_TAXONOMY, $pagenum, $tagsperpage, $searchterms );
							?>
							</tbody>
							</table>
						...
						//if ( $can_manage ) 
						{
							...
							<div class="form-wrap">
								...
								<form name="addmediatag" id="addmediatag" method="post" class="add:the-list: validate"
									action="<?php echo get_option('siteurl') ?>/wp-admin/upload.php?page=<?php echo WPNJ_ADMIN_MENU_KEY; ?>">
	...
	<?php wpnj_inline_edit_mediatags_row('edit-wpnj-media-tags'); ?>
	<?php
}

function wpnj_media_tags_register_columns() {
	...
	register_column_headers('edit-wpnj-media-tags', $media_tags_edit_columns );
}


function wpnj_media_tags_display_rows( $taxonomy = '', $page = 1, $pagesize = 20, $searchterms = '' ) {
	...
	$count = 0;
	foreach( $tags as $tag )
		$out .= _wpnj_media_tag_row( $tag, ++$count % 2 ? ' class="iedit alternate"' : ' class="iedit"' );
...
function _wpnj_media_tag_row( $tag, $class = '' ) {
	$base_url = get_option('siteurl')."/wp-admin/upload.php?page=". WPNJ_ADMIN_MENU_KEY;
	...
	$qe_data = get_term($tag->term_id, WPNJ_MEDIA_TAGS_TAXONOMY, object, 'edit');
	//$edit_link = $base_url ."&action=editmediatag&amp;mediatag_ID=$tag->term_id";
	$edit_link = wpnj_get_mediatag_admin_edit_link( $tag->term_id );
	$view_link = wpnj_get_mediatag_link($tag->term_id);

		
	$out = '';
	$out .= '<tr id="tag-' . $tag->term_id . '"' . $class . '>';
	$columns = get_column_headers('edit-wpnj-media-tags');
	$hidden = get_hidden_columns('edit-wpnj-media-tags');
		...
		switch ($column_name) {
			...
			case 'name':
				...
				$actions['delete'] = "<a class='submitdelete' href='" . wp_nonce_url(get_option('siteurl')
					."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&amp;action=deletemediatag&amp;mediatag_ID=$tag->term_id", 
...
function wpnj_inline_edit_mediatags_row($type) {

	if ( ! current_user_can( 'manage_categories' ) )
		return;

	$is_tag = $type == 'edit-wpnj-media-tags';
...
function wpnj_mediatags_process_actions()
{
	...
	switch($_REQUEST['action'])
	{
		case 'inline-save-mediatag':
			wpnj_mediatags_process_inline_save();
			exit;
			break;
			
		case 'updatemediatag':
			wpnj_mediatags_process_update();
			break;
			
		case 'deletemediatag':
			wpnj_mediatags_process_delete();
			break;

		case 'deletemediatagsbulk':
			wpnj_mediatags_process_delete_bulk();
			break;
			
		case 'addmediatag':
			wpnj_mediatags_process_add();
			break;
...
function wpnj_mediatags_process_add()
{		
	...
	if ( !term_exists( $media_tag_name, WPNJ_MEDIA_TAGS_TAXONOMY ) ) 
	{
		$ret = wp_insert_term( $media_tag_name, WPNJ_MEDIA_TAGS_TAXONOMY, array('slug' => $media_tag_slug));
		if ( $ret && !is_wp_error( $ret ) ) {
			wp_redirect(get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&message=1");
		} else {
			wp_redirect(get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&message=4");
		}
		exit;
	}
}

function wpnj_mediatags_process_delete()
{
	if (!isset($_REQUEST['mediatag_ID']))
		return;
		
	$mediatag_ID = intval($_REQUEST['mediatag_ID']);
	wp_delete_term( $mediatag_ID, WPNJ_MEDIA_TAGS_TAXONOMY);
	
	$redirect_url = get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&message=2";
...
function wpnj_mediatags_process_delete_bulk()
{
	if ((isset($_REQUEST['delete_media_tags'])) && (is_array($_REQUEST['delete_media_tags'])))
	{
		...
		wp_redirect(get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&message=6");
	}	
	else
		wp_redirect(get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY);
	exit;	
}

function wpnj_mediatags_process_update()
{
	if (!isset($_REQUEST['mediatag_ID']))
		return;
	...
	$ret = wp_update_term($mediatag_ID, WPNJ_MEDIA_TAGS_TAXONOMY, array('slug' => $media_tag_slug, 'name' => $media_tag_name));
	if ( $ret && !is_wp_error( $ret ) ) {
		wp_redirect(get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&message=3");
	} else {
		wp_redirect(get_option('siteurl') ."/wp-admin/upload.php?page=".WPNJ_ADMIN_MENU_KEY."&message=5");
	}
	exit;
}

function wpnj_mediatags_process_edit($mediatag_ID)
{
	...
	$tag = get_term($mediatag_ID, WPNJ_MEDIA_TAGS_TAXONOMY, OBJECT, 'edit');			
...
function wpnj_mediatags_process_inline_save()
{
	//echo "_REQUEST<pre>"; print_r($_REQUEST); echo "</pre>";
	...
	$updated = wp_update_term($id, WPNJ_MEDIA_TAGS_TAXONOMY, $_POST);
	if ( $updated && !is_wp_error($updated) ) 
	{
		$tag = get_term( $updated['term_id'], WPNJ_MEDIA_TAGS_TAXONOMY );
		if ( !$tag || is_wp_error( $tag ) )
			die( __('Tag not updated.') );

		echo _wpnj_media_tag_row($tag);
	} 
	else {
		die( __('Tag not updated.') );
	}
...
function wpnj_mediatag_upload_tab($tabs='')
{
	$tabs['mediatags'] = __('Media Tags');
	return $tabs;
}

function wpnj_media_upload_mediatags()
{
	if ( isset($_POST['send']) ) {
	...
	$errors = null;
	return wp_iframe( 'wpnj_media_upload_mediatags_form', $errors );
}

function wpnj_media_upload_mediatags_form($errors)
{
	...
	<?php
	$mediatag_items = wpnj_get_mediatags();
...
function wpnj_mediatags_settings_panel()
{
	if (isset($_REQUEST))
	{
		//echo "_REQUEST<pre>"; print_r($_REQUEST); echo "</pre>";
	...
	<div class="wrap nosubsub">
		...
		<form class="search-form" method="get" action="<?php echo get_option('siteurl') ?>/wp-admin/options-general.php">
			...
			<p>The Media Tags plugin now supports RSS feed when viewing an archive. For when viewing the URL http://www.mysite.com/wpnj-media-tags/some-tag/ you can now access the RSS listing by adding '/feed' to the end of the URL as in http://www.mysite.com/wpnj-media-tags/some-tag/feed/</p>
...
function wpnj_mediatags_reconcile_counts()
{
	...
	if ($mediatag_items) {
		...
		foreach($mediatag_items as $mediatag_item) {
			...
			$media_attachments =  get_objects_in_term($mediatag_item->term_id, WPNJ_MEDIA_TAGS_TAXONOMY);
			if ($media_attachments) {
				...
				foreach($media_attachments as $media_idx => $media_attachment_id) {
					...
					{
						wpnj_mediatags_delete_attachment_proc($media_attachment_id);	
					}
				}					
			}
		}
...
function wpnj_mediatags_library_column_header( $cols ) {
	$cols[WPNJ_MEDIA_TAGS_TAXONOMY] = "Media Tags";
	return $cols;
}

function wpnj_mediatags_library_column_row( $column_name, $id ) {

	if ( $column_name == WPNJ_MEDIA_TAGS_TAXONOMY ) 
	{
		//$media_attachments =  get_objects_in_term($id, WPNJ_MEDIA_TAGS_TAXONOMY);
		$media_attachments = get_the_terms( $id, WPNJ_MEDIA_TAGS_TAXONOMY );
		if ($media_attachments)
		{
			...
			foreach($media_attachments as $media_idx => $media_attachment)
			{
				...
				$media_tag_list_items .= '<a href="'. 
					wpnj_get_mediatag_admin_library_link( $media_attachment->term_id). '">'.  $media_attachment->name. '</a>';				
			}
...
function wpnj_get_mediatag_admin_edit_link( $mediatag_id ) {
	$base_url = get_option('siteurl')."/wp-admin/upload.php?page=". WPNJ_ADMIN_MENU_KEY;
...
function wpnj_get_mediatag_admin_search_link( $mediatag_id ) {
...
function wpnj_get_mediatag_admin_library_link( $mediatag_id ) {
